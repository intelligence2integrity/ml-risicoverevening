...
file_name_this_file <- 'VWS_Gupta_i2i_ML_doe_train_v003.R'

doe_train <- function(model_source, verbose=0, debug=F, data_version=11, save_model=T){
  # libraries
  library(data.table)
  library(mltools)
  library(stringr)
  ...
  if('verbose'  %in% names(formals('i2i_train'))){
    model  <- i2i_train(train_set, verbose=verbose)
  }else if('feature_importance_name'  %in% names(formals('i2i_train'))){
