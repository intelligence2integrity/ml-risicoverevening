########################################################################################################################
#' Bestandsnaam: VWS_Gupta_i2i_ML_nn_bb_correctievxxx.R
#' Doel:         
...
prepareer_data_train_features <- function(dt_x, dt_w, dt_vz){
...
prepareer_data_train_target <- function(dt_y, dt_w, dt_vz, basis_model_preds){
  ...
  dt_agg_y <- dt_agg_y[, target := target - predictions][, target]
...
i2i_train <- function(train_set,
                      alpha = 1,
                      lambda = 1e8,
                      correctie_versterking = 0.5,
                      base_model_source_path = 'src/models/VWS_Gupta_i2i_ML_nn_v010.R')
{
  print('Dataprep:')
  ...
  dt_train_y <- prepareer_data_train_target(dt_train_y, dt_train_w, dt_train_vz, base_model_preds)
  
  # Prepareer features voor de geselecteerde verzekeraars
  dt_train_x <- prepareer_data_train_features(dt_train_x, dt_train_w, dt_train_vz)
